...
const decency = (((progressSumMp + progressSumPrak) / (full_mp + full_prakt) * 100)) + (rep / 10) + reason

dv.span(`
|  Порядочность студента  |     |
| --- | --- |
| **Изученных мп** | **${progressSumMp}** из **${full_mp}** |
...
| --- | --- | --- | --- | :---: |
| **Методические пособия** | ${full_mp} | ${compliat_mp} | ${progress("mp")}
...
|     |  Кол/Текущ   |  Множитель   | 
| --- | --- | --- |
| **Синопсисы** | ${synop} | x**${0.75 + (0.05 * synop)}** = **0.75**<small> баз. мод.</small> + (**0.05** * **${synop}**<small> кол. синоп.</small>) |
...
| Тема рефераты | Сдача реферата | Смысловая нагрузка | Дата | Очков |
| ------------- |:--------------:|:------------------:|:----:|:-----:|
|               |                |                    |      |       |
|               |                |                    |      |       |
|               |                |                    |      |       |
