...
Перечень всех привилегий и ограничений:
...
Балл порядочности высчитывается по следующей формуле: 
`(((изученные_лекции + выполненные_практики) / (всего_лекций + всего_практики) * 100) + (поведение / 10)) * (0.75 + (0.05 * синопсисы)) + баллы_причины`
...
При хорошем поведении студенту добавляется `+5` баллов к поведению (max. 100)
