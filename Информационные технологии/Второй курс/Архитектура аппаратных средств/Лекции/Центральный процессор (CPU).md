...
> **Ядро процессора** отвечает за большую часть всех функций CPU. Оно выполняет расшифровку, чтение, отправку инструкций другим элементам или принимает инструкции от них. 

Одномоментно ядро способно выполнять только одну команду, происходит это за сотые доли секунд. Таким образом, наличие одного ядра говорит о том, что ПК или сервер будет выполнять все инструкции поочередно. Современное оборудование редко использует одноядерные процессоры, так как в этом случае оно работает очень медленно.
...
Другими словами, это количество бит информации, которые процессор может обрабатывать за один *такт* (операцию). 

Например, размер данных за такт равен 1 байту, процессор считает восьмиразрядным (8 bit), если размер данных 2 байта, то ЦПУ шестнадцатиразрядный (16 bit), при размере 4 байта – процессор тридцатидвухразрядный (32 bit), в случае с 8-байтовым размером данных процессор считается шестидесяти четырёхразрядный (64 bit).

![[Pasted image 20231029211252.png|300]]
...
![[Pasted image 20231029213521.png|600]]
...
![[Pasted image 20231024205430.png|400]]

### Кэш-память процессора

Кэш-память процессора является одной из ключевых характеристик, на которую стоит обратить внимание при выборе. 

>**Кэш-память** — массив сверхскоростной энергозависимой ОЗУ. Является буфером, в котором хранятся данные, с которыми процессор взаимодействует чаще или взаимодействовал в процессе последних операций. 

Благодаря этому уменьшается количество обращений процессора к основной памяти. Этот вид памяти делится на три уровня: L1, L2, L3. 
...
![[Pasted image 20231029213732.png|500]]
...
> Процессор представляет собой небольшую квадратную пластину (чип), внутри которой находятся миллионы транзисторов. 
...
В настоящее время технологии развиваются стремительно, и каждый год появляются новые микропроцессоры. Несмотря на это, основные факторы, влияющие на их развитие, хорошо известны. Понимая их, можно достаточно уверенно прогнозировать основные направления эволюции процессоров в ближайшем будущем. Для этого необходимо определить основную цель развития процессоров, выявить ограничения, наложенные на их архитектуру, и оценить существующие современные подходы к созданию микропроцессоров.

### Устремления и ограничения

> **Основная цель всех разработчиков микропроцессоров** — создать процессор с максимальной производительностью при минимальных затратах как на разработку, так и на производство. 

При этом процессор должен быть максимально универсальным. Только при большом объеме производства можно распределить затраты на разработку модели таким образом, чтобы стоимость одного процессора оставалась разумной. Если же процессор ориентирован на узкую сферу применения, львиную долю его стоимости составят расходы на разработку, а не на производство. Именно поэтому уникальные серверные и специализированные платформы, используемые в оборонной сфере и других специфических задачах, стоят так дорого. В целом, затраты на разработку, например, новой модели Celeron и сложной специализированной структуры, могут быть сопоставимы. Однако цена специализированной системы будет в десятки раз выше по сравнению с обычной.

Создать процессор, оптимизированный под одну задачу, проще всего. В этом случае можно достичь пикового уровня производительности на данной элементной базе. Однако, из-за стремления к универсальности, возникают потери в производительности. Это противоречие является одним из основных факторов, влияющих на развитие процессоров. Другим важным фактором является удобство использования процессора для разработки приложений. В любом сложном проекте технология производства становится ключевым аспектом на определенном этапе. Качественная реализация проекта невозможна без применения специализированных средств контроля за качеством программных продуктов. Это объясняет популярность объектно-ориентированного подхода в языках программирования высокого уровня. Также удобство системы команд на уровне машинных кодов влияет на качество работы. Чем удобнее процессор для разработчиков, тем больше программных продуктов создается под эту платформу, делая её более привлекательной для конечных пользователей.

Таким образом, процессор должен обладать максимальной производительностью и сохранять относительную универсальность для обеспечения массового производства. Он также должен быть удобен для разработки сложных приложений. Учитывая все эти требования, можно анализировать существующие модели, оценивать их перспективы и, в некоторой степени, предсказывать их дальнейшее развитие.

### Основные направления развития

Самым значимым фактором, влияющим на архитектурные решения современных процессоров, является постоянное совершенствование технологии производства. Это ведет к росту уровня интеграции, уменьшению задержек в транзисторах и соединениях, а также снижению энергопотребления при переключении транзисторов. С увеличением уровня интеграции возрастает количество ресурсов на кристалле и повышается тактовая частота, что позволяет увеличивать производительность процессоров. Одним из направлений является увеличение объема внутренней кэш-памяти. Другим направлением является внедрение принципов конвейеризации и параллельной обработки, что позволяет процессорам обрабатывать несколько команд на разных стадиях выполнения одновременно.

Большинство архитектурных решений, накопленных в ходе конкуренции различных компаний, находят применение в новых моделях процессоров. Архитектуры процессоров различных производителей имеют много общего, что приводит к вопросам унификации. Современный процессор — это 64-разрядный суперконвейерный, суперскалярный процессор с RISC-ядром и множеством дополнительных блоков, поддерживающий динамическое исполнение команд. Для эффективной обработки мультимедиа и графических данных система команд современных процессоров расширена за счет специализированных инструкций.

Чтобы унифицировать структуры обработки данных, в современные процессоры включаются специальные блоки для преобразования исходных кодов команд во внутренние машинные команды «исполнительного процессора». Ведутся масштабные исследования по созданию процессоров и компьютеров на основе принципиально новой элементной базы: биполярных молекул, молекул ДНК, квантовых кубитов и света.
...
1. [Как работает процессор.](https://glkam1l.github.io/how.html)
2. [Что такое процессор?](https://www.nic.ru/help/chto-takoe-processor-cpu_11148.html)
3. [Как устроен процессор. Видео.](https://www.youtube.com/watch?v=qIhZrMg3_Tk)
